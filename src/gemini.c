...
bool obterPalavrasGemini(char *word1, char *word2) {
    ...
    const char *prompt_base =
        ...
        "Evite repetir palavras comuns. Retorne apenas as duas palavras, em minúsculas e separadas por vírgula. Exemplo: fruto,leito";

    char prompt[1024];
    snprintf(prompt, sizeof(prompt), "%s [%d]", prompt_base, aleatorio);
        ...
        if (text_start) {
            ...
            if (text_end) {
                ...
                if (token1 && token2) {
                    limparToken(token1);
                    limparToken(token2);
                    if (strlen(token1) == 5 && strlen(token2) == 5) {
                        strcpy(word1, token1);
                        strcpy(word2, token2);
                        success = true;
                    } else {
                        fprintf(stderr, "⚠️ Tokens limpos inválidos: '%s', '%s'\n", token1, token2);
                    }
                }
            }
        }
